...
contract SwapFeeReward is Ownable{
    ...
    //max amount in HSW that this contact will be mint;
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'HSWSwapFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'HSWSwapFactory: ZERO_ADDRESS');
    }
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = uint(1000).sub(IHSWPair(pairFor(tokenA, tokenB)).swapFee());
    }
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IHSWFactory(factory).getPair(outputToken, anchorToken) != address(0) && checkPairExist(outputToken, anchorToken)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IHSWFactory(factory).getPair(outputToken, intermediate) != address(0) && IHSWFactory(factory).getPair(intermediate, anchorToken) != address(0) && checkPairExist(intermediate, anchorToken)) {
