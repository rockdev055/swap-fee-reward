...
contract Oracle {
    ...
    function update(address tokenA, address tokenB) external {
        require(msg.sender == priceUpdater, 'HSWOracle: Price can update only price updater address');
        address pair = pairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, 'HSWOracle: PERIOD_NOT_ELAPSED');
        (uint price0Cumulative, uint price1Cumulative,) = HSWOracleLibrary.currentCumulativePrices(pair);
